...
(function (fn) {
  ...
  var lazyIds = {},
    inView = function (el, off) {
      var $el = $(el),
        ...
        width =
          (window.innerWidth || document.documentElement.clientWidth) + off;
      return (
        ...
        var id = pex.randomId(lazyIds),
          fn = function () {
            if ($el.is(":visible") && inView($el, 100)) {
              ...
              pex.loadImage(src).done(function (src) {
  ...
  var applyBlockClasses = function ($block, action) {
    var actionName = action === "hide" ? "hide" : "show",
      ...
      reverceActionClasses = $block.data(
        (action === "hide" ? "show" : "hide") + "BlockClass"
      );
    if (actionClasses) {
        ...
        if (value) {
          if (typeof value === "function") {
            plugin.prototype[name] = function () {
              return pex.plugin.provideOption(
                this,
                name,
  ...
  pex.action.set("field-wheel-spin", function ($el) {
    $el
      .off(".fieldWheelSpin")
      .on(
        "mousewheel.fieldWheelSpin DOMMouseScroll.fieldWheelSpin",
        function (e) {
          e.preventDefault();
          e.stopPropagation();
          spinFieldValue(
    ...
    $el
      ...
      .on({
        ...
        "keyup.arrowsSpin": function (e) {
          ...
          if (!activeKey || activeKey !== e.keyCode) {
            return;
          }
          if ($el.data("spinKeyInterval")) {
            clearInterval($el.data("spinKeyInterval"));
  ...
  pex.action.set("show-block", function ($el, opts) {
    if ($el.is("input, option")) {
      ...
      $checks.on(eventName, function () {
        pex.block.action(
          $el.data("blockName"),
          $el.is(":checked") || $el.is(":selected") ? "show" : "hide"
        );
      });
    } else {
  ...
  Tabs.prototype = {
    getCurrentTab: function () {
      return this.opts.current === false
        ? $([])
        : this.$tabs.filter('[data-tab="' + this.opts.current + '"]');
    },
  };

  pex.plugin.provideOptions(
    Tabs,
    {
      current: function (context, name, val) {
        ...
        if (next && current !== next) {
          var $currTab = context.$tabs.filter('[data-tab="' + next + '"]'),
            $currContent = context.$tabsContent.filter(
              '[data-tab-content="' + next + '"]'
            );
          if ($currTab.length && $currContent.length) {
            context.$root.trigger("tabPreShow", [
              context,
              $currTab,
              $currContent,
            ]);
            ...
            if (current) {
              context.$tabsContent
                .filter('[data-tab-content="' + current + '"]')
                .addClass("hiding")
  ...
  pex.action.set("active-tab-line", function ($el) {
    var $tabs = $el.closest('[data-action-role="tabs"]'),
      fn = function ($currTab) {
        if ($currTab.length) {
          $el.css("width", $currTab.width()).offset({
      ...
      var fn = function () {
        $el.addClass("hide").after(
          $("<span>")
            ...
            .css("backgroundImage", "url(" + $el.attr("src") + ")")
        );
      };
      if ($el.is("[data-lazy]")) {
  ...
  $.fn.pexInit = function (findFn, silent) {
    var _findFn =
        findFn && $.inArray(findFn, ["filter"]) > -1 ? findFn : "find",
      i;
    if (silent) {
